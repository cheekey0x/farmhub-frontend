...
import { CreateResponse, TCreatePayload } from "./staking-lock.types";
import Decimal from "decimal.js";

const prefix = "/staking-lock";
...
const update = async (
  ...
  const res = await wrapResponse(
    api.put<CreateResponse>(
      `${prefix}/?id=${project.cardanoSetting?.tokenStaking?.custodialLock?._id}`,
      updatePayload
    )
  );
  if (res.ok) {
...
const mappingProjectToPayload = (
  ...
  const basePayload = {
    ...
    lockingTokenId: project.token?.policyId,
