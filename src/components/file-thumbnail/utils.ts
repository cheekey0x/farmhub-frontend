...
const FORMAT_CLOUD_IMAGE = "digitaloceanspaces.com/image";

const iconUrl = (icon: string) => `/assets/icons/files/${icon}.svg`;

// ----------------------------------------------------------------------

export function fileFormat(param: string | undefined) {
  let format;
  const fileUrl = param?.toLocaleLowerCase();
  if (fileUrl && fileUrl?.indexOf(FORMAT_CLOUD_IMAGE) > -1) {
    return "image";
  }
